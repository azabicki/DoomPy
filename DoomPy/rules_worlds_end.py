from globals_ import plr, worlds_end, traits_df, status_df
...
def calc_WE_points(p):
    ...
    match worlds_end['played']:
        case "Algal Superbloom":
            ...
            traits_left = plr['trait_pile'][p+1] if p+1 < len(plr['trait_pile']) else plr['trait_pile'][0]
        ...
        case "Ice Age":
            # -1 each_red
            points = -1 * sum('red' in status_df.loc[trait].color.lower()
                              for trait in trait_pile)
        ...
        case "Retrovirus":
            # -1 each_green
            points = -1 * sum('green' in status_df.loc[trait].color.lower()
                              for trait in trait_pile)
        ...
        case "Solar Flare":
            # -1 each_purple
            points = -1 * sum('purple' in status_df.loc[trait].color.lower()
                              for trait in trait_pile)
        ...
        case "Super Volcano":
            # -1 each_blue
            points = -1 * sum('blue' in status_df.loc[trait].color.lower()
                              for trait in trait_pile)
        ...
        case "The Big One":
            ...
            ncol = []
            for col in colors:
                ncol.append(all([col not in status_df.loc[trait].color.lower()
                            for trait in trait_pile]))
            points = -2 * sum(ncol)
