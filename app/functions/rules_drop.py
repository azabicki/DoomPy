    ...
    match traits_df.loc[trait_idx].trait:
        ...
        case "Bitter Berries":
            ...
            for c in ["green", "purple"]:
                n_cols.append(
                    sum([c in status_df.loc[t].color.lower() for t in trait_pile])
                )
            dp = min(n_cols)
            ...
            for tp in plr["trait_pile"]:
                n_pairs.append(
                    int(
                        sum(
                            "green" in c.lower()
                            for c in status_df.iloc[tp].color.tolist()
                        )
                        / 2
                    )
                )
        ...
        case "Elven Ears":
            ...
            dp = sum(
                traits_df.loc[t].game == "Mythlings"
                for tp in plr["trait_pile"]
                for t in tp
            )
        ...
        case "Euphoric":
            ...
            dp = 6 * (
                sum("purple" in status_df.loc[t].color.lower() for t in trait_pile) >= 4
            )
        ...
        case "Fortunate (1)":
            ...
            if (
                n_col.index(max(n_col)) == colors.index("green")
                and n_col.count(max(n_col)) == 1
            ):
                dp = 2
            else:
                dp = 0
        ...
        case "Nano":
            ...
            if host != "none":
                if not isinstance(status_df.loc[host].face, str):
                    ...
                    print(
                        "___________________ check this out!!! why am i here???? ___________"
                    )
        ...
        case "Opposable Thumbs":
            ...
            copy_idx = [
                i
                for i in trait_pile
                if (traits_df.loc[i].dominant == 1 and i != trait_idx)
            ][0]
        ...
        case "Pollination":
            ...
            dp = sum(face == 1 for face in status_df.iloc[trait_pile].face.tolist())
        ...
        case "Prolific":
            ...
            dp = 6 * (
                sum(
                    "green" in color.lower()
                    for color in status_df.loc[trait_pile].color
                )
                >= 4
            )
        ...
        case "Prowler":
            ...
            if played_by != [] and played_by[0] != p:
                ...
                if status_df.loc[trait_idx].effects == "none":
                    ...
                    prowler_dp = [
                        int(i) if i.lstrip("-").isnumeric() else np.nan
                ...
                for col in colors:
                    n_cols_ref.append(
                        sum(
                            col in color.lower()
                            for color in status_df.iloc[
                                plr["trait_pile"][played_by[0]]
                            ].color.tolist()
                        )
                    )
...
def drop_points(p: int) -> int:
    ...
    if prowler_idx != []:
        prowler_played_by = [
            i for i, tp in enumerate(plr["trait_pile"]) if prowler_idx[0] in tp
        ]
    ...
    if oppo_idx != []:
        ...
        if (oppo_at != []) and (oppo_at[0] != p):
            ...
            copy_idx = [
                i
                for i in plr["trait_pile"][oppo_at[0]]
