    ...
    match what[0]:
        ...
        case 'select':
            match what[1]:
                case 'deck':
                    if log_to_terminal:
                        print(">>> select trait <<< in DECK -> selected trait = '{}' (id:{})".format(*args))
                    f.write(">>> select trait <<< in DECK -> selected trait = '{}' (id:{})\n".format(*args))

                case 'trait_pile':
                    if log_to_terminal:
                        print(">>> select trait <<< in TRAIT PILE -> selected trait = '{}' is selecting '{}' (id:{})".format(*args))
        ...
        case 'play':
            match what[1]:
                ...
                case 'error_2dominants':
                    if log_to_terminal:
                        print(">>> play <<< ERROR - already 2 dominants in '{}'s trait pile".format(*args))
                    f.write(">>> play <<< ERROR - already 2 dominants in '{}'s trait pile\n".format(*args))

                case 'error_no_attachables':
                    if log_to_terminal:
                        print(">>> play <<< ERROR - no traits in '{}'s trait pile for attachment to attach to".format(*args))
        ...
        case 'move':
            match what[1]:
                ...
                case 'move_to':
                    if log_to_terminal:
                        print(">>> move <<< '{}' (id:{}) {} is moved from '{}' to '{}'".format(*args))
        ...
        case 'attach_to':
            match what[1]:
                ...
                case 'detached':
                    if log_to_terminal:
                        print(">>> attachment <<< detached '{}' (id:{}) from host...".format(*args))
                    f.write(">>> attachment <<< detached '{}' (id:{}) from host...\n".format(*args))

                case 'still_detached':
                    if log_to_terminal:
                        print(">>> attachment <<< '{}' (id:{}) still without host...".format(*args))
                    f.write(">>> attachment <<< '{}' (id:{}) still without host...\n".format(*args))

                case 'attached':
                    if log_to_terminal:
                        print(">>> attachment <<< '{}' attached '{}' (id:{}) to '{}' (id:{})".format(*args))
        ...
        case 'remove':
            match what[1]:
                ...
                case 'hand':
                    if log_to_terminal:
                        print(">>> remove <<< '{}' is moving '{}' (id:{}) to his/her hand".format(*args))
                    f.write(">>> remove <<< '{}' is moving '{}' (id:{}) to his/her hand\n".format(*args))

                case 'discard':
                    if log_to_terminal:
                        print(">>> remove <<< '{}' is discarding '{}' (id:{})".format(*args))
                    f.write(">>> remove <<< '{}' is discarding '{}' (id:{})\n".format(*args))

                case 'discard_attachment':
                    if log_to_terminal:
                        print("   >>> remove <<< attachment '{}' (id:{}) is also discarded automatically".format(*args))
        ...
        case 'traits_WE':
            match what[1]:
                case 'reset':
                    if log_to_terminal:
                        print(">>> traits world end <<< resetting '{}'s (id:{}) worlds-end-effect...".format(*args))
                    f.write(">>> traits world end <<< resetting '{}'s (id:{}) worlds-end-effect...\n".format(*args))

                case 'set':
                    if log_to_terminal:
                        print(">>> traits world end <<< setting '{}'s (id:{}) worlds-end-effect to '{}'".format(*args))
        ...
        case 'catastrophe':
            match what[1]:
                ...
                case 'error_same_catastrophe':
                    if log_to_terminal:
                        print(">>> catastrophe <<< ERROR - same catastrophe (#{}) selected as before: '{}'".format(*args))
                    f.write(">>> catastrophe <<< ERROR - same catastrophe (#{}) selected as before: '{}'\n".format(*args))

                case 'catastrophe':
                    if log_to_terminal:
                        print(">>> catastrophe <<< played catastrophe #{}: '{}' (id:{})".format(*args))
        ...
        case 'MOLs':
            match what[1]:
                ...
                case 'MOL':
                    if log_to_terminal:
                        print(">>> MOL <<< played MOL #{} by {}: '{}' (id:{})".format(*args))
                    f.write(">>> MOL <<< played MOL #{} by {}: '{}' (id:{})\n".format(*args))

                case 'MOL_points':
                    if log_to_terminal:
                        print(">>> MOL <<< '{}'s MOL '{}' (id:{}) scores {} points".format(*args))
        ...
        case 'update_trait_status':
            match what[1]:
                case 'reset':
                    if log_to_terminal:
                        print(">>> trait's status <<< '{}' (id:{}) is reseted to defaults".format(*args))
                    f.write(">>> trait's status <<< '{}' (id:{}) is reseted to defaults\n".format(*args))

                case 'attachment':
                    if log_to_terminal:
                        print(">>> trait's status <<< '{}' (id:{}) is updated due to attachment effects of '{}' (id:{})".format(*args))
                    f.write(">>> trait's status <<< '{}' (id:{}) is updated due to attachment effects of '{}' (id:{})\n".format(*args))

                case 'traits_WE':
                    if log_to_terminal:
                        print(">>> trait's status <<< '{}' (id:{}) is updated due to TRAITS_we effects of '{}' (id:{})".format(*args))
                    f.write(">>> trait's status <<< '{}' (id:{}) is updated due to TRAITS_we effects of '{}' (id:{})\n".format(*args))

                case 'WE_effect':
                    if log_to_terminal:
                        print(">>> trait's status <<< '{}' (id:{}) is updated due to WORLDS END effects of '{}'".format(*args))
        ...
        case 'stars':
            match what[1]:
                ...
                case 'epic':
                    if log_to_terminal:
                        print("   >>> dominant <<< EPIC fills both spots in '{}'s trait pile".format(*args))
        ...
        case 'scoring':
            match what[1]:
                case 'update':
                    if log_to_terminal:
                        print(">>> scoring <<< '{}'s current points: face={} | drops={} | WE={} | MOL={} | total={}".format(*args))
                    f.write(">>> scoring <<< '{}'s current points: face={} | drops={} | WE={} | MOL={} | total={}\n".format(*args))

                case 'manual_drops':
                    if log_to_terminal:
                        print(">>> scoring <<< manual drops of {} (id:{}) set to: {}".format(*args))
        ...
        case 'genes':
            match what[1]:
                case 'trait':
                    if log_to_terminal:
                        print(">>> genes <<< '{}'s '{}' (id:{}) has gene effect off '{}' on '{}' -> current effect: {}".format(*args))
                    f.write(">>> genes <<< '{}'s '{}' (id:{}) has gene effect off '{}' on '{}' -> current effect: {}\n".format(*args))

                case 'catastrophe':
                    if log_to_terminal:
                        print(">>> genes <<< catastrophe '{}' has gene effect off '{}' -> current effect: {}".format(*args))
                    f.write(">>> genes <<< catastrophe '{}' has gene effect off '{}' -> current effect: {}\n".format(*args))

                case 'denial':
                    if log_to_terminal:
                        print(">>> genes <<< 'Denial' (id:{}) protects '{}' from gene effect of '{}' -> current effect: {}".format(*args))
                    f.write(">>> genes <<< 'Denial' (id:{}) protects '{}' from gene effect of '{}' -> current effect: {}\n".format(*args))

                case 'denial_t4h':
                    if log_to_terminal:
                        print(">>> genes <<< 'Denial' (id:{}) could not protect '{}' from '{}' -> current effect: {}".format(*args))
                    f.write(">>> genes <<< 'Denial' (id:{}) could not protect '{}' from '{}' -> current effect: {}\n".format(*args))

                case 'spores':
                    if log_to_terminal:
                        print(">>> genes <<< 'Spores' (id:{}) has an gene effect (+1) on '{}' -> current effect: {}".format(*args))
                    f.write(">>> genes <<< 'Spores' (id:{}) has an gene effect (+1) on '{}' -> current effect: {}\n".format(*args))

                case 'sleepy':
                    if log_to_terminal:
                        print(">>> genes <<< 'Sleepy' (id:?) has an gene effect on '{}' by {} -> current effect: {}".format(*args))
                    f.write(">>> genes <<< 'Sleepy' (id:?) has an gene effect on '{}' by {} -> current effect: {}\n".format(*args))

                case 'total_effect':
                    if log_to_terminal:
                        print("   >>> total gene effect: {} -> current gene pools are {}".format(*args))
        ...
        case 'trait_effects':
            match what[1]:
                case 'amatoxins':
                    if log_to_terminal:
                        print(">>> trait effects <<< 'Amatoxins' (id:{}) effect is based on {} discarded colors -> drop = {}".format(*args))
                    f.write(">>> trait effects <<< 'Amatoxins' (id:{}) effect is based on {} discarded colors -> drop = {}\n".format(*args))

                case 'ironwood_protecting':
                    if log_to_terminal:
                        print(">>> trait effects <<< '{}' (id:{}) is protecting {} green trait(s)".format(*args))
                    f.write(">>> trait effects <<< '{}' (id:{}) is protecting {} green trait(s)\n".format(*args))

                case 'ironwood_not_protecting':
                    if log_to_terminal:
                        print(">>> trait effects <<< '{}' (id:{}) does not protect anymore".format(*args))
                    f.write(">>> trait effects <<< '{}' (id:{}) does not protect anymore\n".format(*args))

                case 'meek_protecting':
                    if log_to_terminal:
                        print(">>> trait effects <<< '{}' (id:{}) is protecting {} red trait(s)".format(*args))
                ...
                case 'prowler':
                    if log_to_terminal:
                        print(">>> trait effects <<< 'Prowler' (id:{}) acts on 'color_count' in '{}'s trait pile -> drop = {}".format(*args))
                    f.write(">>> trait effects <<< 'Prowler' (id:{}) acts on 'color_count' in '{}'s trait pile -> drop = {}\n".format(*args))

                case 'shiny':
                    if log_to_terminal:
                        print(">>> trait effects <<< 'Shiny' (id:{}) acts on '{}'s 'colorless' traits -> drop = {}".format(*args))
                    f.write(">>> trait effects <<< 'Shiny' (id:{}) acts on '{}'s 'colorless' traits -> drop = {}\n".format(*args))

                case 'spores':
                    if log_to_terminal:
                        print(">>> trait effects <<< 'Spores' (id:{}) adds {} gene(s) to '{}'s gene pool".format(*args))
                    f.write(">>> trait effects <<< 'Spores' (id:{}) adds {} gene(s) to '{}'s gene pool\n".format(*args))

                case 'viral':
                    if log_to_terminal:
                        print(">>> trait effects <<< 'Viral' (id:{}) acts on '{}' traits in '{}'s trait pile -> drop = {}".format(*args))
