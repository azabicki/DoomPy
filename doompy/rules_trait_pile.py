...
def amatoxins(
    ...
    # add trait
    tk.Label(
        frame, text=trait_name.upper(), fg=cfg["color_purple"], font='"" 14 bold'
    ).grid(row=0, column=0, padx=(10, 0), sticky="ens")
    # add effect
    tk.Label(frame, text=" discarded", image=images["bgpr"], compound=tk.LEFT).grid(
        row=0, column=1, sticky="wns"
    )
    # add ?/drop icon
    ...
    if traits_WE != "none":
        ...
        # add points icon
        tk.Label(frame, image=images[WE_drops]).grid(row=0, column=3)
...
def prowler(
    ...
    # add effect
    tk.Label(frame, text="less ", image=images["bgpr"], compound=tk.RIGHT).grid(
    ...
    # add drop
    tk.Label(frame, image=images["drops"]).grid(row=0, column=3)
    # add points icon
    WE_drops = status_df.loc[trait_idx].effects.split()
    tk.Label(frame, image=images[WE_drops[p]]).grid(row=0, column=4)

    write_log(
        ["trait_effects", "prowler"], trait_idx, plr["name"][p].get(), WE_drops[p]
    )
...
# --- SHINY -------------------------------------------------------------------
def shiny(
    frame_trait_overview: tk.Frame, p: int, irow: int, images, trait_idx: int
) -> int:
    # create separate frame
    irow += 1
    frame = tk.Frame(frame_trait_overview)
    ...
    # add trait
    tk.Label(frame, text="SHINY", fg=cfg["color_green"], font="'' 14 bold").grid(
        row=0, column=0, padx=(10, 0), sticky="ens"
    )
    # add effect
    tk.Label(frame, text="punishes ", image=images["c"], compound=tk.RIGHT).grid(
        row=0, column=1, sticky="wns"
    )
    # add drop icon
    tk.Label(frame, image=images["drops"]).grid(row=0, column=2)
    # add points icon
    shiny_dp = status_df.loc[trait_idx].effects.split()
    tk.Label(frame, image=images[shiny_dp[p]]).grid(row=0, column=3)
...
def spores(
    ...
    if n_genes > 0:
        ...
        # add trait
        tk.Label(frame, text="SPORES", fg=cfg["color_green"], font="'' 14 bold").grid(
            row=0, column=0, padx=(10, 0), sticky="ens"
        )
        # add effect
        tk.Label(frame, text="adds genes ").grid(row=0, column=1, sticky="wns")
        # add gene_pool icon(s)
        for i in range(n_genes):
            tk.Label(frame, image=images["gene_pool"]).grid(row=0, column=2 + i)
...
def viral(
    ...
    # add effect
    lbl = tk.Label(
        frame, text=" punishes ", image=images["question_mark"], compound=tk.RIGHT
    )
    ...
    if trait_WE != "none":
        ...
        tk.Label(frame, image=images["drops"]).grid(row=0, column=2)
        # add points icon
        tk.Label(frame, image=images[WE_drops[p]]).grid(row=0, column=3)
...
def special_trait_effects(
    frame_trait_overview: tk.Frame, p: int, irow: int, images: dict
) -> int:
    ...
    if (
        ...
        irow = amatoxins(
            frame_trait_overview, irow, images, amatoxins_idx[0], "Amatoxins"
        )
    ...
    if spores_idx != []:
        ...
        if spores_effect != "none":
            irow = spores(
                frame_trait_overview, p, irow, images, spores_effect, spores_idx[0]
            )
    ...
    if (
        ...
        match first_D:
            case "Amatoxins":
                irow = amatoxins(
                    frame_trait_overview, irow, images, oppo_idx[0], "Opp. Thumbs"
                )

            case "Prowler":
                irow = prowler(
                    frame_trait_overview, p, irow, images, oppo_idx[0], "Opp. Thumbs"
                )
