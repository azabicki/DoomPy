    ...
    match task:
        case "is_color_of_choice":
            ...
            if str(trait_idx) in prev_WE_effects:
                ...
                prev_cleaned = [
                    i for i in prev_WE_effects.split() if str(trait_idx) not in i
                ]
                status_df.loc[trait_idx, "effects_traits_WE"] = (
                    " ".join(prev_cleaned) if prev_cleaned != [] else "none"
                )
            ...
            if traits_WE != "none":
                ...
                if prev_WE_effect == "none":
                    status_df.loc[trait_idx, "effects_traits_WE"] = (
                        str(trait_idx) + ":" + traits_WE
                    )
                else:
                    new_effect = [
                        str(trait_idx) + ":" + traits_WE if str(trait_idx) in i else i
        ...
        case "may_change_color":
            ...
            for tp_trait in trait_pile:
                ...
                if str(trait_idx) in prev_WE_effects:
                    ...
                    status_df.loc[tp_trait, "effects_traits_WE"] = (
                        " ".join(prev_cleaned) if prev_cleaned != [] else "none"
                    )
                ...
                if traits_WE != "none":
                    ...
                    if col_from.lower() in status_df.loc[tp_trait].color.lower():
                        # skip, if trait itself has a "is_color*" WE_task
                        # -> they should not interfere
                        if (
                            isinstance(traits_df.loc[tp_trait].worlds_end_task, str)
                        ...
                        if prev_WE_effects == "none":
                            status_df.loc[tp_trait, "effects_traits_WE"] = (
                                str(trait_idx) + ":" + effect
                            )
                        else:
                            new_effect = [
                                (
                                    str(trait_idx) + ":" + effect
                                    if str(trait_idx) in i
                                    else i
                                )
                                for i in prev_WE_effects.split()
                            ]
                            status_df.loc[tp_trait, "effects_traits_WE"] = " ".join(
                                new_effect
                            )
                else:
                    status_df.loc[trait_idx, "effects_traits_WE"] = traits_WE
        ...
        case "is_color_of_most_colors":
            ...
            if traits_WE != "none":
                ...
                if prev_WE_effect == "none":
                    ...
                    new_effect = [
                        str(trait_idx) + ":" + effect if str(trait_idx) in i else i
                        for i in prev_WE_effect.split()
                    ]
                    status_df.loc[trait_idx, "effects_traits_WE"] = " ".join(new_effect)
            else:
    ...
    for trait_idx in tp:
        ...
        match trait:
            ...
            case "Ironwood":
                ...
                n_green = sum(
                    "green" in status_df.iloc[t].color.lower()
                    for t in tp
                ...
                if n_green <= 5:
                    ...
                    for t in tp:
                        ...
                        if (
                            ...
                            and (
                                not status_df.loc[t].no_remove or str(t) in prev_effects
                            )
                        ):
                    ...
                    # log
                    write_log(
                        ["trait_effects", "ironwood_protecting"],
                        trait,
                        trait_idx,
                        len(new_effects),
                    )
                    ...
                    if prev_effects != "none":
                        ...
                        # log
                        write_log(
                            ["trait_effects", "ironwood_not_protecting"],
                            trait,
                            trait_idx,
                        )
            ...
            case "Meek":
                ...
                if n_red <= 3:
                    ...
                    for t in tp:
                        # if red & not already no_remove = True
                        if "red" in status_df.iloc[t].color.lower() and (
                            not status_df.loc[t].no_remove or str(t) in prev_effects
                        ):
                            status_df.loc[t, "no_remove"] = True
                            new_effects.append(t)
                    ...
                    if prev_effects != "none":
                        ...
                        # log
                        write_log(
                            ["trait_effects", "meek_not_protecting"], trait, trait_idx
                        )
